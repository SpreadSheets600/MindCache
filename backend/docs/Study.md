...
**How we use it in MindCache:** Webpages visited in the browser are captured by the browser extension, sent to the FastAPI backend via the `/visit` endpoint defined in [visit.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/api/visit.py), cleaned of noise, processed for keywords and entities, indexed locally into both FAISS (vector) and BM25 (lexical) indices, and finally searched through a hybrid ranking algorithm that fuses semantic and lexical signals.
...
**How we use it in MindCache:** MindCache stores all relational metadata — documents, visit history, keywords, extracted entities, search queries, and search clicks — in `data/mindcache.db` using SQLAlchemy ORM models defined in [document.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/models/document.py). The async SQLAlchemy session management is handled in [session.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/db/session.py).
...
- `POST /visit` in [visit.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/api/visit.py) — ingests a URL for processing
- `POST /search` and `POST /search/click` in [search.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/api/search.py) — performs semantic search and records click feedback
- `GET /documents` and `GET /documents/{id}` in [documents.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/api/documents.py) — lists and retrieves stored documents
- `GET /health` in [health.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/api/health.py) — live diagnostics across all system components
...
**How we use it in MindCache:** Tokenization with split-hyphen support is implemented in [bm25_service.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/services/bm25_service.py) (line 26-30) to handle compound query terms like "state-of-the-art" or "stop-slop". All text ingested into the BM25 index goes through this normalization pipeline before token frequency scoring.
...
**How we use it in MindCache:** TF-IDF is used as a lightweight fallback keyword extractor in [keyword_extractor.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/services/keyword_extractor.py). When the local Ollama LLM is unavailable (offline or model missing), the system falls back to scikit-learn's `TfidfVectorizer` to extract top keywords purely from term frequency statistics, ensuring keyword extraction still works even without AI.
...
**How we use it in MindCache:** MindCache calls the local Ollama embeddings API (running models like `embeddinggemma:300m`) in [embedding_service.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/services/embedding_service.py) to generate dense vector representations of document chunks. Embeddings are generated in batches, cached for repeated queries, and stored in the FAISS index for fast similarity search.
...
When comparing documents using word counts, longer documents will naturally have higher word frequencies. Cosine similarity allows you to compare documents of entirely different sizes by evaluating only whether they talk about the same concepts, regardless of how much they say about them.

Two documents can be 100 words and 10,000 words respectively, but if they discuss the same topics, their vectors will point in similar directions and the cosine similarity will be high.

**How we use it in MindCache:** Vectors are L2-normalized before being added to the FAISS index. This means the inner product (dot product) between normalized vectors equals the cosine similarity. The FAISS search in [vector_service.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/services/vector_service.py) uses `IndexFlatIP` (Inner Product) on normalized vectors, which is mathematically equivalent to cosine similarity.

---

### 10. Vector Search

![Vector Search Diagram](../../.github/docs/vector_search.png)
...
**How we use it in MindCache:** Query vectors are searched against chunk vectors in the FAISS index to find the top $K$ semantically matching document chunks. The actual search call is in [vector_service.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/services/vector_service.py) in the `search_similar()` method (line 126), which handles normalization, search, and result extraction.
...
FAISS is not a full-fledged database. It does not natively handle data storage, complex metadata filtering, or user access management. Developers typically store raw data and metadata in traditional databases and use FAISS solely as an in-memory or on-disk index for fast vector searching.

**How we use it in MindCache:** MindCache uses FAISS in [vector_service.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/services/vector_service.py) to manage its vector store locally at `data/faiss_index.bin`. The index uses `IndexIDMap` wrapping `IndexFlatIP` (Inner Product) on L2-normalized vectors. This configuration gives exact cosine similarity search — not approximate — which is appropriate for MindCache's desktop-scale dataset (thousands to low millions of vectors). The index is persisted to disk and loaded on server startup, with automatic dimension-mismatch detection and re-indexing if the embedding model changes.
...
**How we use it in MindCache:** The search pipeline in [search_service.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/services/search_service.py) executes both searches in parallel, then scores each candidate document with a weighted fusion formula:
...
- **S** = Source type boost (GitHub > YouTube/Reddit > generic)

Additionally, click feedback from `/search/click` adds a scored boost of +0.10 per click (capped at +0.30) to previously clicked documents.
...
**How we use it in MindCache:** MindCache extracts platform-specific metadata in specialized extractors — [github.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/services/extractors/github.py) (stars, topics, language), [youtube.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/services/extractors/youtube.py) (channel, views, duration), [reddit.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/services/extractors/reddit.py) (subreddit, score, comments). This metadata is injected into every chunk before vector generation in [document_processor.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/services/document_processor.py) (lines 266-299 for the chunk construction).
...
1. **Embedding model context limits** — most embedding models have a maximum input size (e.g. 512 tokens for many BERT-based models, or several thousand tokens for newer models like `embeddinggemma:300m`). Text beyond this limit is silently truncated, losing semantic information.
...
**How we use it in MindCache:** Performed in [document_processor.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/services/document_processor.py) (lines 273-299). Documents are split into chunks of 3000 characters with 500-character overlap. Each chunk is prepended with rich metadata (title, domain, source type, keywords, entities, platform metadata) before being embedded individually. All chunks from the same document share the same database ID, enabling document-level deduplication during search.
...
flowchart TB
    subgraph "Training Phase"
        T[Teacher: BERT-Base<br/>110M parameters] --> S[Student: DistilBERT<br/>66M parameters]
        D[Training Data] --> T
        D --> S
        T -- "soft labels & hidden states" --> S
    end
```
...
- A relational database (SQLite) for document metadata and relationships
...
**How we use it in MindCache:** A regression search evaluation suite in [test_api.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/tests/test_api.py#L269) reads [search_evaluation.json](https://github.com/SpreadSheets600/MindCache/blob/main/backend/tests/search_evaluation.json) and executes test queries. The suite asserts that expected documents consistently rank at position #1 (MRR = 1.0), providing a regression safety net for ranking algorithm changes.
...
**How we use it in MindCache:** The search pipeline in [search_service.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/services/search_service.py) implements this pattern:

1. **First stage:** FAISS retrieves the top 25 candidates by vector similarity; BM25 retrieves another 25 by lexical match; keyword matches add more
2. **Merge phase:** All candidates are merged into a single pool
3. **Rerank phase:** Each candidate gets a final score computed from 6 weighted signals (vector score, BM25 score, title overlap, keyword overlap, recency, source type), plus a click-boost bonus

The rerank pool size is `max(limit * 3, 15)`, ensuring enough candidates for effective reordering.
...
**How we use it in MindCache:** MindCache runs local RAG entirely offline. When the user enables AI summaries during search, the [search_service.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/services/search_service.py) pipeline gathers the top matching document snippets, formats them into a synthesis prompt, and calls `qwen3.5:2b` via Ollama (handled in [ollama_service.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/services/ollama_service.py) `generate_collective_summary()`). The prompt instructs the model to cite sources by number `[1]`, avoid introductory boilerplate, and synthesize findings into cohesive paragraphs.
...
**How we use it in MindCache:** MindCache logs search clicks via `POST /search/click` in [search.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/api/search.py) to a `search_clicks` table in SQLite. During retrieval, the `search_service` loads click counts for the current query and applies a score boost of +0.10 per click, capped at +0.30. This means a document that has been clicked 3+ times for the same query gets a significant ranking elevation. The system is entirely local and privacy-preserving — no click data ever leaves the machine.
...
**How we use it in MindCache:** The `/graph` endpoint in [documents.py](https://github.com/SpreadSheets600/MindCache/blob/main/backend/app/api/documents.py) (line 224) builds the entire graph dynamically from the SQLite database. It aggregates all documents, entities, and keywords, then computes co-occurrence edges between entities and keywords that appear in shared documents. The frontend renders this as an interactive Canvas-based visualization in [InteractiveKnowledgeGraph.tsx](https://github.com/SpreadSheets600/MindCache/blob/main/extension/src/components/InteractiveKnowledgeGraph.tsx), supporting color-coded entity types, keyword frequency filtering, and multiple coloring modes (Domain, Classic, Recency).
...
**How we use it in MindCache:** _Not currently implemented._ MindCache has all the building blocks for content-based recommendations — stored entity and keyword histories, visit timestamps, and source type metadata. A future feature could surface "forgotten but relevant" documents when you research a topic you explored weeks ago.
...
- **Click boosting** — previously clicked results for the same query receive a rank boost
- **Source type boosting** — GitHub pages outrank generic pages for developer-oriented queries
...
> **Note:** For detailed implementation references and code-level documentation, see the [Architecture](../extension/docs/Architecture.md) and [Development](../extension/docs/Development.md) guides.
